...
module mod3_reduce #(
    ...
    always @(*) begin
        ...
        for (i = 0; i + 1 < W; i = i + 2)
            s = s + x[i +: 2];
        if (W % 2)                       // odd top bit: position W-1 is even,
            s = s + x[W-1];              // weight 4^k == 1 mod 3
        // stage 2..4: fold the byte down (same digit-sum identity)
        t = s[1:0] + s[3:2] + s[5:4] + s[7:6];   // <= 12
        t = {2'b00, t[1:0]} + {2'b00, t[3:2]};   // <= 6
        t = {2'b00, t[1:0]} + {2'b00, t[3:2]};   // <= 3
        // final correction: 3 == 0 (mod 3)
        rr = (t[1:0] == 2'd3 && t[3:2] == 2'd0) ? 2'd0 : t[1:0];
        // (after the folds t <= 3, so t[3:2] is 0; the compare keeps lint honest)
    end
